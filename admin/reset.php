<?php 
  ...
  $code = $_GET['code'];
  $getEmail = mysqli_query($con, "SELECT email FROM password_resets WHERE code ='$code'");
  ...
  $sql1 = "SELECT TIMESTAMPDIFF (SECOND, date, NOW()) AS tdif FROM password_resets WHERE code = '$code'";
  ...
  if ($tdif >= 900) {
    # code...
    $removeQuery = $con->query("DELETE FROM password_resets WHERE code = '$code'");
  ...
  if (isset($_POST['submit'])) {
   ...
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Oops! Passwords did not match! Try aagin </div>";
      ...
      elseif (!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
        # code...
        $msg = "<div class='alert alert-danger alert-dismissible'>
   <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>Password must be 8 characters in length with atleast one uppercase and lowercase letter, one numeric and special character <strong>e.g Sapana!122<strong></div>";
      }
      else {
       
   $password = password_hash($password, PASSWORD_BCRYPT);
     $query = mysqli_query($con, "UPDATE tbladmin SET AdminPassword = '$password' WHERE AdminEmailId = '$emailGot'");
     if ($query) {
       # code...
         $query = $con->query("DELETE FROM password_resets WHERE code = '$code'");
